...
def extract_all_info(source_doc):
    info = {
        "name": "",
        "work_years": "",
        "school": "",
        ...
        for row in table.rows:
            ...
            for i in range(len(cells) - 1):
                ...
                if ("姓" in cell_text and "名" in cell_text) and "工作年限" not in cell_text:
                    ...
                    if not info["work_years"]:
                        info["work_years"] = next_cell_text
                elif "毕业学校" in cell_text and cell_text == "毕业学校":
                    if not info["school"]:
                        info["school"] = next_cell_text
                elif ("专" in cell_text and "业" in cell_text) and "毕业学校" not in cell_text and "最高学历" not in cell_text:
                    if not info["major"]:
...
def create_resume_from_template(template_path, source_resume_path, output_path):
    ...
    personal_info = extract_all_info(source_doc)
