...
## 2026-03-26 - Uzumaru Global-1B.Small 最终回退到 Global-1 基线
- Background: 用户反馈在加入测速、重试、验证等改动后，`/nm` 仍然没有稳定反馈，且回滚链路与启动状态反复出问题。
- Decision: 直接回退到最初的 `Global-1` 干净基线，只保留“切换失败自动回滚”这一项增量；删除测速、状态缓存、启动重试与消息重试，恢复最小可用链路。
- Why: 继续在问题版本上叠加修补只会放大不稳定因素；回到已验证基线是最快恢复可用性的办法。
- Follow-up: 后续任何新增能力都必须先确认不会影响 `/nm` 的最小切换链路，否则默认不加。

## 2026-03-26 - 以后上线统一按版本发布
- Background: 用户要求以后每次上线都按“版本发布”处理，确保出问题可以明确回退。
- Decision: 之后任何开发/上线都先形成一个可回退版本，再部署到线上；每次发布都记录版本基线、改动点和回退路径，不再做无版本标识的直接覆盖。
- Why: 版本化发布能把“当前可用状态”固化下来，线上出问题时可以直接回切已知稳定版，避免边改边上导致无法回退。
- Follow-up: 后续任何开发默认先确认对应版本基线和回退方案，再进行上线。
...
## 2026-03-23 - 服务器巡检记录标准化
- Background: 用户要求把后续服务器检查结果持续同步到 Obsidian，便于统一维护。
- Decision: 对每台服务器固定记录三项：1) 代理进程与对外监听端口；2) 15 秒短窗口抓包是否存在持续入站；3) 是否需要额外安装 `tcpdump` 才能完成抓包。
- Why: 统一“先找端口，再短抓包，再看日志”的排查顺序，便于后续横向对比与复查。
- Follow-up: 后续新服务器记录优先追加到 `decision_log.md`，由 `sync_memory_to_ob.sh` 镜像到 Obsidian。

## 2026-03-23 - 服务器巡检结果汇总
- 87.83.100.252: `xray` 对外端口 `45945`，`x-ui` 监听 `2096/8418`，短抓包未见 `45945` 持续入站。
- 87.83.101.55: `xray` 对外端口 `35724`，短抓包未见持续入站。
- 87.83.97.29: `xray` 对外端口 `17117`，短抓包未见持续入站。
- 87.83.98.201: `xray` 对外端口 `53023`，短抓包未见持续入站。
- 87.83.105.63: `xray` 对外端口 `41110`，短抓包未见持续入站。
- 8.138.120.16: 运行 `gost`，对外端口 `10002`，先安装 `tcpdump` 后短抓包，未见持续入站。
- 43.167.254.6: `xray` 多入站端口 `43293/40002/443/43800/20422/58727`，先安装 `tcpdump` 后对入站窗口抓包，未见持续入站。
- 2a01:4f9:6b:4a8f:47::a: `sing-box` 对外端口 `45724`，先安装 `tcpdump` 后短抓包，未见持续入站。
- 202.73.4.182:40814: `xray` 对外端口 `41002/41003`，先安装 `tcpdump` 后短抓包，未见持续入站。
- 191.96.11.239:41153: SSH 端口为 `41153`，`xray` 对外端口 `41155`，`x-ui` 监听 `2096/41154`，先安装 `tcpdump` 后短抓包，未见持续入站。
- 107.172.231.70:48356: SSH 端口为 `48356`，`sing-box` 对外端口 `52911`，`xray` 对外端口 `52912`，先安装 `tcpdump` 后短抓包，`52912` 出现活跃会话，源 IP 为 `14.145.170.36`。
- 45.127.35.233:48981: SSH 端口为 `48981`，`sing-box` 对外端口 `48982`，先安装 `tcpdump` 后短抓包，出现多条活跃会话，源主机显示为 `202.73.4.182`。
- 140.99.243.151:50203: SSH 端口为 `50203`，`xray` 对外端口 `50204`，先安装 `tcpdump` 后短抓包，未见持续入站。
- 154.7.179.43:2333: SSH 端口为 `2333`，`xray` 对外端口 `25572`，`x-ui` 监听 `2096/25572`，先安装 `tcpdump` 后短抓包，未见持续入站。
- 87.83.100.252: `x-ui` 额外端口 `2096/8418` 短抓包未见持续入站。
- 87.83.101.55: `x-ui` 额外端口 `2096` 短抓包未见持续入站。
- 87.83.97.29: `x-ui` 额外端口 `2096` 短抓包未见持续入站。
- 87.83.98.201: `x-ui` 额外端口 `2096` 短抓包未见持续入站。
- 87.83.105.63: `x-ui` 额外端口 `2096` 短抓包未见持续入站。
- 45.141.36.130: 额外端口 `80/59745` 短抓包未见持续入站。
- 23.141.52.67: 额外端口 `24854` 短抓包未见持续入站。
- 140.99.243.151: 额外端口 `10085` 短抓包未见持续入站。
- 154.7.179.43: 额外端口 `2096` 短抓包未见持续入站。
- 191.96.11.239: 额外端口 `41154` 短抓包未见持续入站。
- 202.73.4.182: 额外端口 `41001` 短抓包未见持续入站。
- 107.172.231.70: `52912` 长抓包 60 秒未再抓到包，但此前与 `14.145.170.36` 的真实会话已确认。
- 45.127.35.233: `48982` 长抓包 60 秒继续出现多条会话，源主机仍显示为 `202.73.4.182`。
- 202.73.4.182: `41003` 长抓包 60 秒持续出现多条会话，源主机仍显示为 `14.145.170.36`。
- 45.127.35.233 的 `sing-box` 日志显示来自 `202.73.4.182` 的 `ss-in` 连接会继续访问 Google / Apple / gstatic 等正常站点，未见 banned 或异常错误，判断为正常代理使用。
- 202.73.4.182 的 `xray` 会话均来自 `14.145.170.36`，连接稳定且无 banned 记录，判断为正常代理使用而非扫描。
- 38.207.191.187:5522: SSH 端口为 `5522`，`xray` 对外端口为 `44312`，`x-ui` 监听 `2096/51685`，`tcpdump` 已可用，15 秒短抓包未见持续入站。
...
## 2026-03-26 - Uzumaru 切换失败自动回滚
- Background: 用户反馈切换某些节点（例如 10 号节点）后会“没反应、像卡住”，且没有自动回到原节点。
- Decision: 为 `tg-exit-bot` 增加失败回滚机制：先切目标节点，再用短超时 `ip.sb` 探测验证；若切换失败或验证失败，自动调用原节点的 `switch_via_api` 回滚，并在 TG 回调里明确提示是否已回到原节点。
- Why: 坏节点会让出口变成不可用状态，若不自动回滚就会让服务器保持在错误出口上；短超时验证可减少“卡住”的体感。
- Follow-up: 后续切坏节点时，先看 TG 是否提示“已自动回到原来的节点”，再查 `/var/log/tg-exit-bot.err` 是否有回滚失败。

## 2026-03-26 - Uzumaru 节点状态与延迟手动刷新
- Background: 用户希望在 TG 手动刷新节点清单时就能看出各节点是否可用，并显示延迟，且不想每次打开 `/nm` 都重新探测。
- Decision: 将 `/nm` 的按钮文案改为读取进程内缓存的探测结果；只有点击“刷新列表”或执行 `/refresh` 时才会先刷新 `out.sh`，再逐个探测节点并缓存 `🟢/🔴 + 延迟/错误`，之后再重建菜单。
- Why: 手动刷新才做探测可以兼顾可见性与速度，避免每次打开面板都因为逐个切换节点而变慢；缓存让 `/nm` 的展示保持轻量。
- Follow-up: 后续查看节点状态先点一次“刷新列表”，然后再用 `/nm` 浏览最新的红绿点和延迟结果。

## 2026-03-26 - Uzumaru 刷新过程分段回报
- Background: 用户反馈节点刷新时长较久，界面长时间停留在“正在更新...”看起来像没有响应。
- Decision: 将刷新探测改成分段更新进度：每隔若干节点就编辑一次 TG 消息，显示已完成数量、在线/离线统计，最后再渲染完整菜单；同时把节点名放到按钮文本最前面，尽量减少视觉偏移。
- Why: 长耗时任务如果没有阶段性反馈，用户会以为任务卡死；分段回报能明显改善体感。
- Follow-up: 后续若仍觉得慢，可以进一步把刷新分成“先出总览、后补状态”的两阶段展示。

## 2026-03-26 - Uzumaru 刷新与测速解耦
- Background: 用户仍觉得刷新列表时等待过久，希望把“更新节点清单”和“探测速度/延迟”拆开。
- Decision: 将 `刷新列表` 改为只更新 `out.sh` 和菜单，不再执行测速；新增独立的 `📡 测速节点` 入口专门做通断与延迟探测，并把按钮文案改为 `🟢 123ms  10. 美国 ATT` 这种醒目顺序。
- Why: 刷新和测速是两种不同意图，合并执行会让刷新体感很慢；拆开后用户可以先快速拿到最新节点，再按需测速。
- Follow-up: 后续默认先点刷新列表，再按需要点测速节点；如果只想换出口，不必等待测速完成。

## 2026-03-26 - Uzumaru 测速流程崩溃修复
- Background: 用户反馈测速界面停在 `0/29` 很久；日志显示测速流程在第一台节点回切时触发 `NameError: _message_text is not defined`。
- Decision: 在 `bot.py` 中补上 `_message_text` 辅助函数，并把测速进度改成每测完一台节点就回写一次当前进度和节点名，避免长时间停在同一屏。
- Why: 这次不是测速本身失效，而是回切后的异常直接中断了循环；补上辅助函数后流程才能继续走完。
- Follow-up: 后续若测速仍很慢，优先先看是否真的在执行 `📡 测速节点`，再按逐节点的进度判断是不是正常耗时。

## 2026-03-26 - Uzumaru 探测源改成多端点 fallback
- Background: 用户反馈测速结果把前面一批实际上可用的节点误判为离线，离线率过高。
- Decision: 将出口连通性探测从单一 `ip.sb` 改为多端点 fallback：先试 `gstatic` 的 204，再试 `Cloudflare` 204，最后才回落到 `ip.sb`；只要任一端点成功就判定在线，并取首个成功端点的延迟作为展示值。
- Why: 单一站点会把站点自身的波动误当成节点故障，导致大量假阴性；多端点 fallback 能显著提高“在线/离线”判断的稳定性。
- Follow-up: 后续如果离线率仍高，优先检查当前可用的探测站点是否整体受阻，而不是先怀疑节点本身。

## 2026-03-26 - Uzumaru 实验节点预检版本发布
- Background: 用户确认要避免切到坏的实验节点后让 bot 失联，并要求以后每次上线都按版本发布、可回退处理。
- Decision: 为 `Global-1B.Small` 发布一个单独版本：只对实验性节点做切换后 Telegram 预检，失败则自动回滚到稳定出口 `香港机房出口`；稳定节点不额外增加预检开销。
- Why: 坏出口问题已经被定位为链路风险，必须把防护限制在实验节点，既能防止失联，又不破坏稳定出口体验。
- Follow-up: 后续如果实验节点再次导致 Telegram 超时，优先看预检结果和回滚日志，而不是先怀疑 bot 主逻辑。

## 2026-03-26 - Uzumaru Global-1.Small 同步实验节点预检版本
- Background: 用户要求将 Global-1.Small 也加上同样的实验节点预检和自动回滚加固，保持两台机器版本一致。
- Decision: 将相同的预检版本同步到 `Global-1.Small`，继续以 `Global-1` 干净基线为主，只对实验节点执行 Telegram 预检，失败自动回滚到稳定出口。
- Why: 两台机保持同版本可以减少“B 有、A 没有”的排障歧义；同时保留清晰的版本发布和回退路径。
- Follow-up: 后续若任一台机器出现切坏失联，先按版本号和备份目录回退，再看是不是出口链路问题。
